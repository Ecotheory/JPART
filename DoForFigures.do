...
*== Figure 1: Kernel Density of SO2 Data in 2008 ==*
	twoway (kdensity Satellite if Connect==1, lcolor(navy) lwidth(medthick) lpattern(solid)) (kdensity Official if Connect==1, lcolor(navy) lwidth(medthick) lpattern(longdash)) (kdensity Satellite if Connect==0, lcolor(maroon) lpattern(solid)) (kdensity Official if Connect==0, lcolor(maroon) lpattern(longdash)) if year==2008, ylabel(, angle(horizontal) format(%7.1f)) title(Kernel Density of SO2 Data (2008), size(medlarge)) scheme(s1color) xtitle("") legend(label(1 "Satellite Connect") label(2 "Official Connect") label(3 "Satellite No-connect") label(4 "Official No-connect"))
	graph export "result/Figure1.png", as(png) name("Graph") replace
	
*== Figure 2: Kernel Density of SO2 Data in 2012 ==*
	twoway (kdensity Satellite if Connect==1, lcolor(navy) lwidth(medthick) lpattern(solid)) (kdensity Official if Connect==1, lcolor(navy) lwidth(medthick) lpattern(longdash)) (kdensity Satellite if Connect==0, lcolor(maroon) lpattern(solid)) (kdensity Official if Connect==0, lcolor(maroon) lpattern(longdash)) if year==2012, ylabel(, angle(horizontal) format(%7.1f)) title(Kernel Density of SO2 Data (2012), size(medlarge)) scheme(s1color) xtitle("") legend(label(1 "Satellite Connect") label(2 "Official Connect") label(3 "Satellite No-connect") label(4 "Official No-connect"))
	graph export "result/Figure2.png", as(png) name("Graph") replace
	
*==Figure 3: Kernel Density of SO2 Manipulation Data ==*
	histogram ln_ManipulationSO2, fcolor(none) lcolor(gray) normal title(Kernel Density of SO2 Manipulation Coefficient , size(medium)) scheme(s1mono)
...
*== Figure 4: The Marginal Effects of Connection on Manipulation ==*
	...
	marginsplot, recast(line) plotopts(lcolor(black)) recastci(rline) ciopts(lpattern(dash) lcolor(black))  level(95)  scheme(s1mono) title(Average Marginal Effects of Connection with 95% CIs, size(medium)  margin(small)) ytitle(Effects on SO2 Manipulation Coefficient, size(medsmall)) xtitle(City Secretary Age, size(medsmall))  ylabel(, format(%12s))
